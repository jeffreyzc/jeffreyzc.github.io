---
layout: page
title: Professional / R&D Projects
---

# Professional / R&D

Selected work across photochemical systems, polymer materials, thin-film fabrication, synchrotron characterization, and prototype/system design.

<style>
.project-row {
  display: flex;
  gap: 28px;
  align-items: flex-start;
  margin-bottom: 60px;
}

.project-image {
  flex: 1;
}

.project-image img {
  width: 100%;
  max-width: 420px;
  border-radius: 8px;
}

.project-text {
  flex: 1.4;
}

.project-text h2 {
  margin-top: 0;
}

@media (max-width: 700px) {
  .project-row {
    flex-direction: column;
  }
}
</style>

<br>


<div class="project-row">

  <div class="project-image">
    <img src="assets/images/thin-film-01.jpg" alt="Semiconductor thin-film fabrication and characterization">
  </div>

  <div class="project-text">
    <h2>Semiconductor Thin-Film Fabrication and Characterization</h2>

    <p><strong>What I did:</strong><br>
    Fabricated and characterized semiconductor thin films and interface-engineered materials, focusing on how processing, morphology, and interfacial structure affect material behavior.</p>

    <p><strong>Methods / tools:</strong><br>
    Thin-film fabrication, coating, XRD, microscopy, spectroscopy, and electronic/material characterization.</p>
  </div>

</div>

<div class="project-row">

  <div class="project-image">
    <img src="assets/images/synchrotron-01.jpg" alt="Synchrotron-based characterization and data analysis">
  </div>

  <div class="project-text">
    <h2>Synchrotron-Based Characterization and Data Analysis</h2>

    <p><strong>What I did:</strong><br>
    Used synchrotron-based characterization and custom data-analysis workflows to study material structure, scattering patterns, and time-dependent responses.</p>

    <p><strong>Methods / tools:</strong><br>
    SAXS/WAXS, X-ray data processing, Python analysis, diffraction/scattering visualization, and quantitative interpretation.</p>
  </div>

</div>
